...
import { UploadCloud, Send, RefreshCw, Loader2, AlertCircle, FileText, Receipt, CreditCard, X} from 'lucide-react';
  ...
  const loadDemoInvoice = async () => {
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        const response = await fetch('/demo/invoice-sample-1.jpg');
        const blob = await response.blob();
        const file = new File([blob], 'demo-invoice.jpg', { type: 'image/jpeg' });
      ...
      <main className="flex-1 bg-gradient-to-b from-gray-50 to-teal-50 text-gray-900">
        <header className="bg-white border-b border-gray-200 shadow-sm py-4">
          <div className="container mx-auto px-4">
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-xl font-semibold text-gray-900">Invoice Data Extraction</h1>
                <p className="text-sm text-gray-500">Upload and analyze individual invoices</p>
              </div>
              <div className="flex gap-2">
                {!imagePreview && !isLoading && (
                  <button
                    onClick={loadDemoInvoice}
                    className="flex items-center gap-2 px-4 py-2 text-sm bg-teal-100 text-teal-800 rounded-md hover:bg-teal-200 transition-colors"
                  >
        ...
        <div className="container mx-auto p-4 flex flex-col md:flex-row gap-6 h-[calc(100vh-8rem)]">
          ...
          {isDemoMode && !imagePreview && (
            <div className="w-full mb-4 p-4 bg-teal-50 border border-teal-200 rounded-lg text-teal-800 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm">Loading a sample invoice for demonstration purposes. You'll be able to ask questions about the invoice details once processing is complete.</p>
              </div>
            </div>
          )}
          ...
          <div className="flex-1 flex flex-col bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
            ...
            <div className="bg-teal-50 p-4 border-b border-gray-200">
              <h2 className="font-medium text-gray-800">Invoice Assistant</h2>
            ...
            <div ref={chatContainerRef} className="flex-1 p-4 overflow-y-auto">
              {chatHistory.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-400">
                  ...
                  <p className="text-center">Upload an invoice to start extracting data</p>
                </div>
              ) : (
              ...
              <div className="flex items-end gap-2">
                ...
                <div className="relative flex-1">
                  <textarea
                    ...
                    className="w-full border border-gray-300 rounded-lg px-4 py-3 pr-24 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 resize-none overflow-hidden bg-white text-gray-900"
                  ...
                  <div className="absolute right-2 bottom-2 flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-2 text-gray-400 hover:text-teal-600 disabled:text-gray-300 transition-colors"
                    >
          ...
          <div className="w-full md:w-80 flex flex-col gap-4">
            {suggestedQuestions.length === 0 && !imagePreview && !isLoading && (
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-gray-200 bg-teal-50">
                  <h2 className="text-base font-medium text-gray-800">Getting Started</h2>
                </div>
                <div className="p-4">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <button
                      onClick={triggerFileUpload}
                      className="text-left p-3 bg-gray-50 hover:bg-teal-50 hover:text-teal-700 border border-gray-200 hover:border-teal-200 rounded-lg transition-all text-sm flex items-center gap-2"
                    >
                      <UploadCloud className="h-4 w-4" />
                      <span>Upload your invoice</span>
                    </button>
                    <button
                      onClick={loadDemoInvoice}
                      className="text-left p-3 bg-gray-50 hover:bg-teal-50 hover:text-teal-700 border border-gray-200 hover:border-teal-200 rounded-lg transition-all text-sm flex items-center gap-2"
                    >
                      ...
                      <span>Try with a demo invoice</span>
                    </button>
                  </div>
                </div>
              </div>
            ...
            {suggestedQuestions.length > 0 && (
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-gray-200 bg-teal-50">
                  <h2 className="text-base font-medium text-gray-800">Invoice Questions</h2>
                </div>
                <div className="p-4">
                  <div className="flex flex-col gap-2">
                    {suggestedQuestions.map((question, index) => (
                      <button
                        ...
                        disabled={isLoading}
                        className="text-left p-3 bg-gray-50 hover:bg-teal-50 hover:text-teal-700 border border-gray-200 hover:border-teal-200 rounded-lg transition-all text-sm disabled:opacity-70 disabled:hover:bg-gray-50 disabled:hover:text-inherit"
                      >
                        {question}
                      </button>
                    ))}
